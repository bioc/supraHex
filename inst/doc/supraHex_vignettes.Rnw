...
\title{\Rpackage{supraHex}: a supra-hexagonal map for analysing tabular omics data}
...
\Rpackage{supraHex} is a package for the R computing environment and it is assumed that you have already installed the latest version of R. You can install the package from the source codes following step-by-step guidelines in \url{http://supfam.org/SUPERFAMILY/dcGO/supraHex.html}. Or, you can obtain it from Bioconductor:
<<eval=F, echo=T>>=
...
The functions in the package \Rpackage{supraHex} are divided into two categories: one for training and analysis (see Table~\ref{tab:train}), the other for visualisation (see Table~\ref{tab:visual}). To get help information for the package, please type one of two commands:
<<eval=F, echo=T>>=
...
This dataset\footnote{\url{http://www.ncbi.nlm.nih.gov/pubmed/21402797}} contains gene expression levels (15243 genes and 7 embryo stages) (see Figure~\ref{supraHex_vignettes-plant}).
...
data(Xiang)
# transform data by row/gene centering
data <- Xiang - matrix(rep(apply(Xiang,1,mean),ncol(Xiang)),ncol=ncol(Xiang))
# get trained
sMap <- sPipeline(data)
visHexMulComp(sMap, title.xy=c(0.35,1), zlim=c(-1,1), gp=grid::gpar(cex=0.8))
...
# get reordered
sReorder <- sCompReorder(sMap, amplifier=3, metric="pearson")
visCompReorder(sMap, sReorder, title.xy=c(0.35,1), zlim=c(-1,1), gp=grid::gpar(cex=0.8))
\end{verbatim}
