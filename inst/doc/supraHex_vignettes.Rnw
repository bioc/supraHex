...
\title{\Rpackage{supraHex}: a supra-hexagonal map for analysing high-dimensional omics data}
...
\vspace{12pt}
\begin{abstract}
\noindent We introduce an R/Bioconductor package called \Rpackage{supraHex}\footnote{\url{http://supfam.org/SUPERFAMILY/dcGO/supraHex.html}}. It names after a supra-hexagon that is a giant hexagon on a 2-dimensional (2D) map grid seamlessly consisting of smaller hexagons. This 2D giant hexagon is intended to train, analyse and visualise a high-dimensional omics data, which usually involves tens of thousands of genomic coordinates (e.g. genes) but at most hundreds of samples. The resulting supra-hexagon ensembles the structure of the input data in a topology-preserving fashion. Thanks to the prevalence in nature and symmetric beauty, the supra-hexagon is suited for data exploratory analysis plus the ease with visualisation. In this vignette/guide, we give a tutorial-style introduction into how the functions contained in the package \Rpackage{supraHex} can be used to better understand the input data. This vignette assumes some basic familiarity with the \texttt{R} language and environment. It only provides a task-oriented description of the package functionality, and compliments the accompanying manual 'supraHex-manual.pdf' that gives a full description of all functions.
\end{abstract}
...
The \Rpackage{supraHex} is a supra-hexagonal map. It consists of smaller hexagonal lattices on a regular 2-dimensional (2D) grid; these smaller hexagons collectively form a giant hexagon (see Figure~\ref{supraHex_vignettes-suprahex}). To be sure that a supra-hexagon is forme dexactly, inherent relationships must be met between the total number $nHex$ of hexagons in the grid, the grid radius $r$, and the xy-dimensions $xdim$ and $ydim$: 
...
dev.off()
@
\myincfig{supraHex_vignettes-suprahex}{0.6\textwidth}{A supra-hexagonal map}{It consists of $nHex=169$ smaller hexagons. For easy reference, these hexagons are indexed according to: firstly, how many steps a hexagon is away from the grid centroid ('1' for the centroid itself); secondly, for those hexaons of the same step, an anti-clock order starting from the rightmost. This map can also be easily described by the grid radius (i.e., the maximum steps away from the centroid; $r=8$ in this case), or by the xy-dimensions of the map grid (i.e., the maximum number of hexagons horizontally/vertically; $xdim=ydim=15$ in this case).}

%-------------------------------
\subsection{Why to use \Rpackage{supraHex}?}\label{subsec:why}
%-------------------------------

Biologists are far ofter confronted with ever-increasing amounts of omics data that are tabulated in the form of matrix, measuring levels/activities of genomic coordinates (e.g. genes) against experimental samples. This matrix usually involves tens of thousands of genes but a much smaller number of samples (at most hundreds). The atypical structure 'small-sample-but-large-gene' requires easy-to-interpret models. Learning algorithm such as self-organising map is popular in this aspect for its unique capability in capturing the input data, this is, performing vector quantisation but regularised by vector projection. The \Rpackage{supraHex} borrows this learning algorithm to produce a supra-hexagonal map for input omics data. Thanks to the prevalence in nature and symmetric beauty, this supra-hexagonal map is probably suited for analysing data with approximately perfect symmetry in high-dimensional input space. We argue that omics data tend to be symmetric for global-scale measurements of gene levels/activities. Even when priori knowledge of the data structure is unknown, at least the supra-hexagonal map can provide the ease with visualisation.

%-------------------------------
\subsection{How to interpret \Rpackage{supraHex}?}\label{subsec:how}
%-------------------------------

After training, the supra-hexagonal map ensembles the structure of the input data in a topology-preserving fashion, that is, preserving the shape and density of the input data. Each map node is associated with two coordinate systems: one in 2-dimensional space just as you see; the other in input high-dimensional space as you imagine. The coordinate in this high-dimensional space is represented as a prototype vector, collectively forming the codebook matrix. The simplest explanation is: the codebook matrix ensembles the input data matrix. In terms of gene activity matrix as input, \Rpackage{supraHex} produces a map, wherein (i) genes with the same or similar activity patterns are located/clustered to the same or nearby map nodes, (ii) the density of genes in this map follows the data density in the high-dimensional space, and (iii) when all map nodes are color-coded according to values in each component of all prototype vectors (i.e. the column of codebook matrix), the color-coded component map can be used to illustrate sample-specific gene activities. In essence, the supra-hexagonal map trains/models the input data that is converted into the codebook matrix.
...
\Rpackage{supraHex} is a package for the R computing environment and it is assumed that you have already installed the latest version of R (\url{http://www.r-project.org}). You can install the package from the source codes following step-by-step guidelines in \url{http://supfam.org/SUPERFAMILY/dcGO/supraHex.html}. Or, you can obtain it from Bioconductor (\url{http://www.bioconductor.org}):
<<eval=F, echo=T>>=
source("http://bioconductor.org/biocLite.R")
biocLite("supraHex",depend=TRUE)
@

As with any R package, to use \Rpackage{supraHex} you should first load it: 
<<eval=T, echo=T>>=
library(supraHex) # load the package
@

\Rpackage{supraHex} is free to use and redistribute under GPL 2. If you use this package for a publication, we would ask you to cite: 
<<eval=F, echo=T>>=
...
To get general information for the package, the command is:
<<eval=F, echo=T>>=
library(help="supraHex")
@

Alternatively, type the following command and follow the links to the \Rpackage{supraHex} package help. 
<<eval=F, echo=T>>=
help.start()
@
...
\underline{It is equivalent to meta-gene clustering}. Write out results into a tab-delimited text file using \Rfunction{sWriteData}:
...
You will see: the first column for your input data ID (an integer; otherwise the row names of input data matrix), the second column for the corresponding index of best-matching hexagons (i.e. gene clusters), and the third column for the cluster bases (i.e. meta-gene clusters).

%-------------------------------
\subsection{Get reordered using \texttt{sCompReorder} and \texttt{visCompReorder}}\label{subsec:reorder}
%-------------------------------

Reordering components for trained map can be realised by using a new map grid (with sheep shape consisting of a rectangular lattice) to train component plane vectors (either column-wise vectors of codebook/data matrix or the covariance matrix thereof). As a result, similar component planes are placed closer to each other. The functions \Rfunction{sCompReorder} and \Rfunction{visCompReorder} are respectively to implement this reordering algorithm and to visualise the reordered compments (see Figure~\ref{supraHex_vignettes-sReorder})\footnote{In order to display colors properly, it is important to reset the argument 'zlim' by respecting the range of input data matrix (more precisely, codebook matrix). By default, it is set to c(0,1). The simplest way is to first choose the range from the global minimum of your input data matrix to the global maximum, and then to fine-tune this choose for the best display.}. 

<<results=hide, echo=T>>=
sReorder <- sCompReorder(sMap=sMap, amplifier=2, metric="cos")
visCompReorder(sMap=sMap, sReorder=sReorder, zlim=c(-1.5,1.5))
@
...
visHexMulComp(sMap_ga, zlim=c(-1.5,1.5))
@
...
visHexMulComp(sMap_bu, zlim=c(-1.5,1.5))
@
...
visHexMulComp(sMap_cu, zlim=c(-1.5,1.5))
@
...
visHexMulComp(sMap_ep, zlim=c(-1.5,1.5))
@
...
visHexMulComp(sMap_gm, zlim=c(-1.5,1.5))
@
...
data <- Golub
# get trained
sMap <- sPipeline(data, neighKernel="ep")
visHexMulComp(sMap,colormap="yr",title.xy=c(0.35,1),zlim=c(8,12),gp=grid::gpar(cex=0.8))
sWriteData(sMap, data, filename="Output_leukemia.txt")
# get visualised
visHexMapping(sMap, mappingType="indexes")
visHexMapping(sMap, mappingType="hits")
visHexMapping(sMap, mappingType="dist")
# get reordered
sReorder <- sCompReorder(sMap,amplifier=2,metric="pearson")
visCompReorder(sMap,sReorder,colormap="yr",title.xy=c(0.35,1),zlim=c(8,12),
...
# get trained
sMap <- sPipeline(data)
visHexMulComp(sMap, title.xy=c(0.35,1), zlim=c(-1,1), gp=grid::gpar(cex=0.8))
sWriteData(sMap, data, filename="Output_embryo.txt")
# get visualised
visHexMapping(sMap, mappingType="indexes")
visHexMapping(sMap, mappingType="hits")
visHexMapping(sMap, mappingType="dist")
...
sWriteData(sMap, data, sBase, filename="Output_base_embryo.txt")
# get reordered
sReorder <- sCompReorder(sMap, amplifier=3, metric="none")
visCompReorder(sMap, sReorder, title.xy=c(0.35,1), zlim=c(-1,1), gp=grid::gpar(cex=0.8))
\end{verbatim}
