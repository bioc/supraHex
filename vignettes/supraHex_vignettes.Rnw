...
\Rpackage{supraHex} is a package for the R computing environment and it is assumed that you have already installed the latest version of R. You can install the package following step-by-step guidelines in \url{http://supfam.org/SUPERFAMILY/dcGO/supraHex.html}. Briefly, you can install it from Bioconductor:
...
# get trained
sMap <- sPipeline(data)
visHexMulComp(sMap,colormap="darkgreen-lightgreen-lightpink-darkred",title.rotate=10)
...
visCompReorder(sMap,sReorder,
colormap="darkgreen-lightgreen-lightpink-darkred",title.rotate=15)
\end{verbatim}
...
# get trained
sMap <- sPipeline(data)
visHexMulComp(sMap,colormap="darkblue-white-darkorange",title.rotate=10)
...
visCompReorder(sMap,sReorder,colormap="darkblue-white-darkorange",title.rotate=10)
\end{verbatim}
